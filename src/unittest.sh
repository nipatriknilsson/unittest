...
if [ "${arg_help}" == "1" ] ; then
    echo "Copyright (c) 2020 Patrik Nilsson, MIT License"

    echo ""

    echo "unittest [-p] [-f max_count] [object-files...] [-- options to the compiler]"
...
$(cat ${testfile}.funcs | awk -F ',' '{print "extern const int " $NF "();"}')
...
${testfile} ${arg_single} -f ${arg_max} $argtestfile
